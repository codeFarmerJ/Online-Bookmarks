...
?>
    ...
    require_once (ABSOLUTE_PATH . "bookmarks.php");
        $query = "SELECT title, url, description, UNIX_TIMESTAMP(date) AS timestamp, id, favicon FROM bookmark
                WHERE user=? AND childof=? AND deleted!='1' AND public='1' ORDER BY $order[1]";
        $args = [$user, $folderid];
...
<?php
	...
	$query = "SELECT user, SUM(bookmarks) AS bookmarks, SUM(folders) AS folders FROM (
              SELECT user, 1 AS bookmarks, 0 AS folders FROM bookmark WHERE public='1' AND deleted!='1'UNION ALL
              SELECT user, 0 AS bookmarks , 1 AS folders FROM folder WHERE public='1' AND deleted!='1') AS tmp GROUP BY user";
