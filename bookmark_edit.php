...
//		if ($mysql->query ($query)) {
		$query = "SELECT title, id, public, favicon FROM bookmark WHERE id IN (?) AND user=? ORDER BY title";
		$args = [$gbmlist, $username];
...
//					$mysql->escape ($username));
//		if ($mysql->query ($query)) {
		$query = "UPDATE bookmark SET public=? WHERE id IN (?) AND user=? ";
		$args = [$post_public, $bmlist, $username];
...
//				$mysql->escape ($username));
//	if ($mysql->query ($query)) {
	$query = "SELECT title, url, description, childof, id, favicon, public FROM bookmark 
		WHERE id=? AND user=? AND deleted != '1' ";
	$args = [$bmlist[0], $username];
	if ($result = $mysql->query ($query,$args)) {

//		if (mysql_num_rows ($mysql->result) != 1) {
		if ( $result->rowCount() != 1) {
			$cc = $result->rowCount();
			message ("No Bookmark to edit. $cc $bmlist[0] $username  .");
		}
		else {
...
//					if (!$mysql->query ($query)) {
					...
					$args = [$post_public, $favicon->favicon, $username, $bmlist[0] ];
					if ($mysql->query ($query,$args)) {
						message ($mysql->error);
					}
				}
...
//				$mysql->escape ($username));
//	if ($mysql->query ($query)) {
	$query = "UPDATE bookmark SET title=?, url=?, description=?, childof=?, public=?
				WHERE id=? AND user=? ";
	$args = [$post_title, $post_url, $post_description, $post_childof, $post_public, $bmlist[0], $username];
	if ($mysql->query ($query,$args)) {
		echo "Bookmark successfully updated<br>\n";
		echo '<script language="JavaScript">reloadclose();</script>';
	}
