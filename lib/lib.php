...
function check_username ($username) {
	...
	if (isset ($username) || $username == '') {
		global $mysql;
		$query = "SELECT COUNT(*) as cnt FROM user WHERE md5(username)=md5(?) ";
		$args = [$username];
...
function admin_only () {
	...
	$query = "SELECT COUNT(*) as cnt FROM user WHERE admin='1' AND username=? ";
	$args = [$username];
	$result = $mysql->query ($query,$args)->fetch(PDO::FETCH_ASSOC);
	if ($result['cnt'] == 1) {
		$return = true;
...
function print_footer () {
	...
	echo '0.8.15_beta by Stefan Frech.</br>';

	echo '<a class="footer" href="#" target="_blank">Online-Bookmarks</a>' . "\n";
...
function object_count () {
	...
	$query = "SELECT (SELECT COUNT(*) FROM bookmark WHERE user=?) AS bookmarks,
                    (SELECT COUNT(*) FROM folder   WHERE user=?) AS folders ";
