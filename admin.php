...
<div style="min-width: <?php echo 230 + $settings['column_width_folder']; ?>px;">
	...
	if ($create == 'Create') {
		if ($new_username == '' || $new_password == '') {
			...
			$query = "INSERT INTO user (username, password, admin) VALUES (?, md5(?), ?)";
			$args = [$new_username, $new_password, $new_admin ];
				...
				<div style="border: 1px solid #bbb; margin: 10px; padding: 10px;">
					...
					if ($delete == 'Delete') {
						if (check_username ($existing_user)) {
							if ($noconfirm) {
								$query = "DELETE FROM user WHERE md5(username)=md5(?)";
								$args = [$existing_user];
								if ($mysql->query ($query, $args)) {
								...
								$query = "DELETE FROM bookmark WHERE md5(user)=md5(?)";
								$args = [$existing_user];
								if (!$mysql->query ($query, $args)) {
									message ($mysql->error);
									}
					
								$query = "DELETE FROM folder WHERE md5(user)=md5(?)";
								$args = [$existing_user];
								if (!$mysql->query ($query, $args)) {
									message ($mysql->error);
									}
